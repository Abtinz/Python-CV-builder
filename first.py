personalInformation ={
    "name": "" ,
    "lastname" : "",
    "age" : 0 ,
    "city" : "",
    "gender" : "",
    "phonenumber" : 0,
     "birthday" : {
        "day" : 0,
        "month" : 0,
        "year" : 0
     },
    "social media" : []
}

skillsInformation = []
carrierInformation = []

login = {
   "Email" : "",
   "Username" : "",
   "Password" : "",
}

def mainMenu() :
   print("welcome to CV builder")
   print("1) lets make your personal CV ")
   print("2) view profile")
   print("Else number : Exit")
   chioce = int(input())
   if chioce == 1 :
      signup()
      CVmaker()
   elif chioce == 2 :
      logIn()  
   else :
      print("good bye ^ _ ^")

def signup() :
        login["Email"] = input("Email : ")
        login["Username"] = input("Username : ")
        login["Password"] = input("password : ")
        while 1==1 :
          loginPassword=input("enter your password again : ")
          if loginPassword in login["Password"] :
             break
          else :
           print("its wrong !")
           print("1) try again")
           print("2) i cant remeber !" +"\n"+"lets sign up again ")
           chioce = int(input())
           if chioce == 2 :
              signup()
 
def CVmaker() :
   print("1) personal information")
   print("2) skilles information")
   print("3) carrer information")
   print("4) main menu")
   chioce = int(input())
   if chioce == 1 : personal_info()
   elif chioce == 2 : skilles_info()    
   elif chioce == 3 : carrer_info()  
   elif chioce == 4 : mainMenu()
   else : CVmaker()

def personal_info() :
    
   for key in  personalInformation :
      if key == "age" :
          detail=int(input(key + ':'))
          personalInformation[key]= detail
      elif key == "phonenumber" :
          detail= int(input(key + ':'))
          personalInformation[key]= detail
      elif key == "birthday" :
            print("birthday information")
            year=int(input("year :"))
            personalInformation["birthday"]["year"] = year
            while 1==1 :
               month = int(input("month :"))
               if month <1 and month > 12 :
                  print("invalid number try again !")
               else :
                  personalInformation["birthday"]["month"]=month
                  break
            while 1==1 :  
               day=int(input("day :"))
               if personalInformation["birthday"]["month"] <7 :
                  if day > 31 and day <1 :
                     print("invalid number try again !")
                  else :
                     personalInformation["birthday"]["day"] = day
                     break 
               elif personalInformation["birthday"]["month"] > 6 and personalInformation["birthday"]["month"] < 12 :
                  if day > 30 and day <1 : 
                     print("invalid number try again !")
                  else :
                     personalInformation["birthday"]["day"] = day
                     break      
               else :
                   if day > 29 and day <1 : 
                     print("invalid number try again !")
                   else :
                     personalInformation["birthday"]["day"] = day 
                     break   
      elif key == "social media" :
        while 1 == 1 :
         personalInformation[key].append(input(key + ':'))
         print("do you want to enter other social media ?") 
         print("1) yes")
         print("etc. no") 
         choice = int(input())
         if choice != 1 :
          break       
      else :
          detail=input(key + ':')
          personalInformation[key]= detail
   CVmaker()

def skilles_info() :
        while 1 == 1 :
         skillsInformation.append(input("new skill : "))
         print("do you want to enter other skill ?") 
         print("1) yes")
         print("etc. no") 
         choice = int(input())
         if choice != 1 :
             break 
        CVmaker()
def carrer_info() :
         while 1 == 1 :
          carrierInformation.append(input("new carrier : "))
          print("do you want to enter other carrier ?") 
          print("1) yes")
          print("etc. no") 
          choice = int(input())
          if choice != 1 :
              break               
         CVmaker()
def logIn() :
   
   while 1==1 :
          loginemail=input("Email : ")
          if loginemail in login["Email"] :
             break
          else :
           print("its wrong !")
   
   while 1==1 :
          loginusername=input("User name : ")
          if loginusername in login["Username"] :
             break
          else :
           print("its wrong !")
      
   while 1==1 :
          loginPassword=input("password : ")
          if loginPassword in login["Password"] :
             break
          else :
           print("its wrong !")

   profilemenu()

def profilemenu() :
   print("1) view informations")
   print("2) edit informations")
   print("3) main menu")
   chioce = int(input())
   if chioce == 1 : viewInformations()     
   elif chioce == 2 : editInformations()     
   elif chioce == 3 : mainMenu()
   else : profilemenu()
      
def  viewInformations() :
     n=1
     print("Personal informations :")
     for key in personalInformation.keys() :
        print(f"{n}) {key} : {personalInformation[key]}")
        n-=-1
     print("\n"+"Skills informations :")
     n=1   
     for item in skillsInformation :
        print(f"{n}) {item}")
        n-=-1
     print("\n"+"Carrier informations :")
     n=1   
     for item in carrierInformation :
        print(f"{n}) {item}")
        n-=-1
     print("Press 1 to exit")   
     choice = int(input())
     if choice == 1 : profilemenu()
     else : viewInformations()


def editInformations() :
   viewInformations()
   print("1) change Personal informations") 
   print("2) Skills informations") 
   print("3) Carrier informations")
   print("4) Exit") 
   choice = int(input())
   if choice == 1 : personalEdit()
   elif choice == 2 : skillesEdit()
   elif choice == 3 : carrerEdit()
   elif choice==4 : profilemenu()
   else : mainMenu()

def personalEdit() :
   print("Personal informations :")
   n=1
   for keys in personalInformation.keys() :
        print(f"{n}) {keys} : {personalInformation[keys]}")
        n-=-1

   while 1==1 :
     key=input('please enter your changing subject : ')
     if key in personalInformation.keys() : break
     else : print("its wrong !")
   
   if key == "age" :
          detail=int(input(key + ':'))
          personalInformation[key]= detail
   elif key == "phonenumber" :
          detail= int(input(key + ':'))
          personalInformation[key]= detail
   elif key == "birthday" :
            print("birthday information")
            year=int(input("year :"))
            personalInformation["birthday"]["year"] = year
            while 1==1 :
               month = int(input("month :"))
               if month <1 and month > 12 :
                  print("invalid number try again !")
               else :
                  personalInformation["birthday"]["month"]=month
                  break
            while 1==1 :  
               day=int(input("day :"))
               if personalInformation["birthday"]["month"] <7 :
                  if day > 31 and day <1 :
                     print("invalid number try again !")
                  else :
                     personalInformation["birthday"]["day"] = day
                     break 
               elif personalInformation["birthday"]["month"] > 6 and personalInformation["birthday"]["month"] < 12 :
                  if day > 30 and day <1 : 
                     print("invalid number try again !")
                  else :
                     personalInformation["birthday"]["day"] = day
                     break      
               else :
                   if day > 29 and day <1 : 
                     print("invalid number try again !")
                   else :
                     personalInformation["birthday"]["day"] = day 
                     break   
   elif key == "social media" :
        while 1 == 1 :
         personalInformation[key].append(input(key + ':'))
         print("do you want to enter other social media ?") 
         print("1) yes")
         print("etc. no") 
         choice = int(input())
         if choice != 1 :
          break       
   else :
          detail=input(key + ':')
          personalInformation[key]= detail 
   print("1) edit another subjec") 
   print("Else number : Exit") 
   choice = int(input())
   if choice == 1 : personalEdit()
   else : editInformations()


        
